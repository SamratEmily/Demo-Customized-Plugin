...
class Task {
    ...
    public function __construct() {
        // Hook for displaying new task template in order details
        add_action( 'dokan_order_detail_after_order_general_details', [ $this, 'add_new_task' ], 10, 2 );

        // Hook for saving the custom task data when the order is processed
        add_action( 'admin_post_save_custom_task', [ $this, 'handle_form_submission' ] ); // Form submission for logged-in users (admin)

        // Hook for displaying the custom task data in the admin order edit page
    ...
    public function handle_form_submission() {
        if ( isset( $_POST['new_task'], $_POST['save_new_task_nonce'], $_POST['order_id'] ) && wp_verify_nonce( $_POST['save_new_task_nonce'], 'save_new_task_action' ) ) {
            // Get the order ID from the form and save the data
            $order_id = intval( $_POST['order_id'] );
            $new_task = sanitize_text_field( $_POST['new_task'] );

            if ( $order_id && $new_task ) {
                // Save the task to order meta (wp_postmeta table)
    ...
    public function display_order_data_in_admin( $order ) {
        ...
        // Optionally display a success message if data was just saved
